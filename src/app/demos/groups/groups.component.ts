...
export class GroupsComponent implements OnInit {
  ...
  private loadEntityListPage(page: Observable<Page>) {
    page.subscribe((page: Page) => {
      this.entities.set(page.elements);
  ...
  trackByEntityId(index: number, entity: Group): number {
    return entity.id;
  }
  ...
  delete() {
    this.groupService.delete(this.idOfentityToDelete).subscribe((val: string) => {
      ...
      this.sharedService.sendShowMsgDiv({
        msgType: 'success',
        msg: 'The group is deleted successfully.',
      });
      this.loadEntityListPage(this.groupService.get$(this.pageNav.currentPage, this.criterias));
      $('#btn_delete2').click();
    });
  ...
  private loadEntityById(id: number) {
    this.entities().forEach((entity: Group) => {
      if (entity.id == id) {
  ...
  getEntities(event) {
    this.loadEntityListPage(
      this.groupService.get$(parseInt(event.target.innerText), this.criterias),
    );
  }
  ...
  getPageLot(forNext: boolean) {
    this.loadEntityListPage(
      this.groupService.get$(
        this.sharedService.getPageNumByChangePageLot(this.pageNav, forNext),
        this.criterias,
      ),
    );
  }
